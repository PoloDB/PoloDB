...
int PLDB_get_collection_id_by_name(Database* db, const char* name);

int64_t PLDB_count(Database* db, uint32_t);

int PLDB_insert(Database* db, uint32_t col_id, const DbDocument* doc);

// <query> is nullable
int PLDB_find(Database* db, uint32_t col_id, const DbDocument* query, DbHandle** out_handle);

// <query> is nullable
int64_t PLDB_update(Database* db, uint32_t col_id, const DbDocument* query, const DbDocument* update);

int64_t PLDB_delete(Database* db, uint32_t col_id, const DbDocument* query);

int64_t PLDB_delete_all(Database* db, uint32_t col_id);

int PLDB_drop(Database* db, uint32_t col_id);
