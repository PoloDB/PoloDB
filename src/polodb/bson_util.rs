...
use crate::sync_read_ext::SyncLittleEndianRead;
...
pub(crate) fn update_document_check(update: &Document) -> Result<()> {
    match first_key(update) {
        Some(key) => {
            if !key.starts_with('$') {
                Err(anyhow!("update document must only contain update modifiers"))
            } else {
                Ok(())
            }
        }
        None => Err(anyhow!("update document must not be empty")),
    }
}
...
pub(crate) fn replacement_document_check(replacement: &Document) -> Result<()> {
    if let Some(key) = first_key(replacement) {
        if key.starts_with('$') {
            return Err(anyhow!("replacement document must not contain update modifiers"));
        }
    }
    Ok(())
}
