...
fn test_delete_issues_127() {
    ...
    // Run #1
    {
        ...
        let result = col.find(None).run().unwrap().collect::<Result<Vec<Document>>>().unwrap(); // The document { "name": "t1" } is returned, but none should be returned instead
...
fn test_delete_issues_148() {
...

    {
        let db = Database::open_path(db_path.as_path()).unwrap();
        let col = db.collection::<Document>("tasks");
        let result = col.find(None).run().unwrap().collect::<Result<Vec<Document>>>().unwrap();
