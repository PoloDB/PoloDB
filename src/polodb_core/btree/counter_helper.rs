...
pub(crate) fn count(page_handler: &mut dyn Session, collection_meta: MetaDocEntry) -> DbResult<u64> {
    let item_size = cal_item_size(page_handler.page_size());
    count_by_btree_pid(page_handler, item_size, 0, collection_meta.root_pid())
}

fn count_by_btree_pid(page_handler: &mut dyn Session, item_size: u32, parent_pid: u32, pid: u32) -> DbResult<u64> {
